...
public class BoardTestSuite {
    ...
    void testAddTaskListFindOutdatedTasks() {
        ...
        * When
        ...
        List<Task> tasks = project.getTaskLists().stream()
//                .filter(tl -> tl.getName().contains(undoneTasks.get(0).getName()) || tl.getName().contains(undoneTasks.get(1).getName()))
                .filter(undoneTasks::contains) // Warning. There is bad filtering in this case!
...
//                .filter(tl -> tl.getName().contains(inProgressTasks.get(0).getName()))
                .filter(inProgressTasks::contains) // Warning. There is bad filtering in this case!
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        * When
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        int[] daysOfCreated = project.getTaskLists().stream()
        ...
        * Then
         */
        assertEquals(10d, getAverage(daysOfCreated));
    }
